	...
	for (const token of tokens) {
		if (token.scopes.some((scope) => scope === 'invalid')) {
			if (spacesOnlyRE.test(sourceCode.substring(lineOffsets[token.startLine] + token.startColumn, lineOffsets[token.endLine] + token.endColumn))) {
				...
				const source = sourceCode.substring(lineOffsets[token.startLine] + token.startColumn, lineOffsets[token.endLine] + token.endColumn);

				failures.push({
					ruleId: 'internalInvalid',
					message: `Invalid expression "${source}"${(tmScope && tmScope in ILLEGAL_SCOPE_MAPPING ? ` in ${ILLEGAL_SCOPE_MAPPING[tmScope]}` : '')}.`,
					startLine: token.startLine,
					startColumn: token.startColumn,
					endLine: token.endLine,
					endColumn: token.endColumn,
