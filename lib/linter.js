...
/** @typedef {{ [ruleName: string]: string | [FailureSeverity | 'off', Object]; }} OptionsRules
...
module.exports = {
		...
		const registry = new vsctm.Registry({
			...
			async loadGrammar (scopeName) {
				if (scopeName === SMARTY_SCOPE_NAME) {
					const grammarFile = './node_modules/@modix/smarty-tmlanguage/smarty.tmLanguage.json';
	...
	async verify (sourceCode, options) {
		...
		if (tokens.length > 1) {
			...
			failures.push(...checkInvalid(sourceCode, tokens, lineOffsets));
