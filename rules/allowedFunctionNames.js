 ...
 * @returns {void}
 */
module.exports = function allowedFunctionNames ({ tree, addError, functions, modifiers }) {
	...
	function parseChildren (parentName, children) {
		for (let i = 0; i < children.length; i++) {
			if (parentName === 'function' && children[i].name === 'identifier') {
				if (!functions.includes(children[i].raw.toLowerCase())) {
					addError({
						message: `Unknown function "${children[i].raw}".`,
				...
				if (!modifiers.includes(children[i].raw.toLowerCase())) {
					addError({
						message: `Unknown modifier "${children[i].raw}".`,
						startLine: children[i].startLine,
						startColumn: children[i].startColumn,
						endLine: children[i].endLine,
						endColumn: children[i].endColumn
